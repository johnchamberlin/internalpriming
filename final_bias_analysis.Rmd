...
ggplot(data.frame(n_genes = c(with(qmarksCell, sum(p_val_adj < .05 & avg_log2FC > 1)),
...
nomalInexL5IT = CreateSeuratObject(cbind(nomalCell,nomalNuc))
nomalInexL5IT$tech = rep(c("cell","nuc"),times=c(ncol(nomalCell),ncol(nomalNuc)))
...
celltype_glbias %>% ggplot(aes(x = R, y = median_log10_marker_length, color = tech)) + geom_point() + scale_color_manual(values=c("gray","orange")) + theme_minimal()
...
cellMats = read_rds("data/star/AIBS_scell_V3/all_cell_mats_5.rds")
nucMats = read_rds("data/star/AIBS_snuc_V3/all_nuc_mats_5.rds")
...
cellMats$int_frac = 1-cellMats$exon/cellMats$inex
